...
// Used as response type for ReadBytesFromArchive and GetContentReaderDataForEvent
// (BytesResponse handles serialization via its own serialize() method)

// Used as response type for GetDocumentArchiveID
// (StringResponse handles serialization via its own serialize() method)

// Used as response type for CreateAudioReader, GetArchiveSize, etc.
// (HandleResponse handles serialization via its own serialize() method)

// Used as response type for WriteBytesToArchive, IsMusicalContextContentAvailable, etc.
// (Int32Response handles serialization via its own serialize() method)
