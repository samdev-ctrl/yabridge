...
// Decode the AraHostInstanceProxy* from a host ref encoded as
// (ara_dc_id << 8) | interface_index.  The pointer is stored in a
// thread-local on first use so the trampoline just passes the encoded ref.
// Actually we embed the pointer directly: each host ref stores its
// AraHostInstanceProxy* cast to uintptr_t, shifted or tagged if needed.
// Simpler: we store the proxy pointer directly in the host ref.
...
ARA::ARAAudioReaderHostRef ARA_CALL AraHostInstanceProxy::create_audio_reader(
    ...
    const uint64_t id = p->bridge_.send_message(
        YaAra::HostCallback::CreateAudioReader{
            ...
            static_cast<int32_t>(use64bit)}).value;
    return reinterpret_cast<ARA::ARAAudioReaderHostRef>(id);
}
...
ARA::ARABool ARA_CALL AraHostInstanceProxy::read_audio_samples(
    ...
    void* const buffers[]) {
    // Shared-memory path implemented in task 10. For now return silence.
    // buffers is non-interleaved; channel count unknown here so zero safely.
    // Caller must know the channel count; we silence all provided pointers.
    // Per the spec: on failure fill with zeros and return kARAFalse.
    (void)count;
    (void)buffers;
...
const void* ARA_CALL AraHostInstanceProxy::get_content_reader_data_for_event(
    ...
    return p->last_event_data_.empty() ? nullptr : p->last_event_data_.data();
}
